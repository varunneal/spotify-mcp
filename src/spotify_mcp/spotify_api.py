    ...
    if not all(config.values()):
        try:
            ...
            if pyproject_path.exists():
                ...
                defaults = pyproject_data.get("tool", {}).get("spotify-mcp", {}).get("env", {})
                config["CLIENT_ID"] = config["CLIENT_ID"] or defaults.get("SPOTIFY_CLIENT_ID")
                config["CLIENT_SECRET"] = config["CLIENT_SECRET"] or defaults.get("SPOTIFY_CLIENT_SECRET") 
                config["REDIRECT_URI"] = config["REDIRECT_URI"] or defaults.get("SPOTIFY_REDIRECT_URI")
        except Exception:
